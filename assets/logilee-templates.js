(function () {
  ...
  const xlsxLayouts = {
    ...
    "packing-list": { file: "packing-list.xlsx", sheet: "PL", title: "PACKING LIST", columns: [1, 3, 5, 7, 8, 9], totalsRow: 74, rows: (d) => (d.rows || []).map((row) => { const factor = weightToKg[row.weightUnit] || 1; return [`${row.packageNo || ""} ${row.marks || ""}`.trim(), row.description, num(row.quantity), num(row.netWeight) * factor, num(row.grossWeight) * factor, `${row.length} x ${row.width} x ${row.height} ${row.dimensionUnit} / ${packageCbm(row).toFixed(3)} CBM`]; }), totals: (d) => { const total = totals("packing-list", d); return [{ column: 5, value: total.packages }, { column: 7, value: total.net }, { column: 8, value: total.gross }, { column: 9, value: total.cbm }]; } },
  ...
  function topValues(id, d) {
    ...
    return { A5: [d.sellerName || d.shipperName, d.sellerAddress || d.shipperAddress].filter(Boolean).join("\n"), A10: [d.buyerName || d.consigneeName, d.buyerAddress || d.consigneeAddress].filter(Boolean).join("\n"), A15: [notifyName, notifyAddress].filter(Boolean).join("\n"), H4: [d.invoiceNo || d.packingNo || d.proformaNo, d.invoiceDate || d.packingDate || d.issueDate].filter(Boolean).join(" / "), H6: [d.lcNo, d.lcDate].filter(Boolean).join(" / "), H9: d.lcBank || "", H12: d.remarks || d.notes || "", E19: route.loading || "", E21: [route.carrier, route.sailing].filter(Boolean).join(" / ") };
  }
